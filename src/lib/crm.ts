...
export type ClientMatch = { contact: string; name: string; jobs: Booking[] };
...
export function formatClient(match: ClientMatch, esc: (s: string) => string): string[] {
  const lines = [
    `👤 <b>${esc(match.name)}</b> · <code>${esc(match.contact)}</code> · ${match.jobs.length} job${match.jobs.length === 1 ? '' : 's'}`,
  ];
